...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--include-background",
        action="store_true",
        help="Also save unmatched detector proposals as a background class.",
    )
    parser.add_argument(
        "--background-class",
        default="background",
        help="Class directory/name used for negative proposal crops.",
    )
    parser.add_argument(
        "--negative-iou",
        type=float,
        default=0.30,
        help="Save a proposal as background when its max IoU with any GT is below this value.",
    )
    parser.add_argument(
        "--max-background-per-image",
        type=int,
        default=8,
        help="Maximum negative/background crops saved per image.",
    )
    parser.add_argument(
        "--max-background-ratio",
        type=float,
        default=1.5,
        help="Cap background crops per split to ratio * positive crops. Use <=0 to disable.",
    )
...
def process_split(
    *,
    ...
    metadata_rows = []
    positive_class_names = [name for name in classes if name != args.background_class]
    stats = {
        "images_seen": 0,
        "images_with_gt": 0,
    ...
    for image_id in image_ids:
        ...
        for pred in predictions:
            ...
            pred_with_iou = {**pred, "max_iou": best_iou}
            if args.include_background and best_iou < args.negative_iou:
                background_candidates.append(pred_with_iou)
                stats["background_candidates"] += 1
        ...
        with Image.open(image_path) as opened:
            ...
            if args.include_background:
                if max_background_for_split is None and args.max_background_ratio > 0:
                    # Lazy cap: positive crop count is only known as we process the split.
                    # The cap is checked against the running positive count so negatives
                    # cannot grow far beyond the positive pool.
                    max_background_for_split = 0
                bg_candidates = sorted(background_candidates, key=lambda item: item["score"], reverse=True)
                bg_candidates = bg_candidates[: args.max_background_per_image]
                for rank, candidate in enumerate(bg_candidates):
                    if args.max_background_ratio > 0:
                        allowed_background = int(max(1, stats["positive_matches"]) * args.max_background_ratio)
                        if stats["background_saved"] >= allowed_background:
                            break
                    label = args.background_class
                    crop_box = expanded_crop_box(
                        candidate["bbox_xyxy"],
                        width,
                        height,
                    ...
                    filename = (
                        f"{Path(image_id).stem}__bg__p{candidate['pred_index']:04d}"
                        f"__r{rank}__{label}.jpg"
                    )
                    crop_path = output / split / label / filename
                    save_crop(image, crop_box, crop_path, args.image_size, args.jpeg_quality)
                    if len(preview_samples[label]) < args.preview_per_class:
                        preview_samples[label].append(crop_path)
                    stats["background_saved"] += 1
                    stats["saved_crops"] += 1
                    class_counts[label] += 1
                    background_iou_values.append(candidate["max_iou"])
                    metadata_rows.append(
                        {
                            "split": split,
    ...
    return metadata_rows, {
        **stats,
        ...
        "positive_class_crop_counts": {name: class_counts[name] for name in positive_class_names},
        ...
        "background_iou_mean": sum(background_iou_values) / len(background_iou_values)
        if background_iou_values
        else 0.0,
        "background_iou_max": max(background_iou_values) if background_iou_values else None,
    }
...
def main() -> None:
    ...
    if not 0 <= args.negative_iou <= 1:
        raise ValueError("--negative-iou must be in [0, 1]")
    if args.negative_iou >= args.positive_iou:
        raise ValueError("--negative-iou must be lower than --positive-iou to leave an ignore band")
    ...
    if args.max_background_per_image <= 0:
        raise ValueError("--max-background-per-image must be positive")
    classes = load_classes(args.classes)
    if args.include_background and args.background_class not in classes:
        raise ValueError(f"--background-class {args.background_class!r} must exist in --classes")
    ...
    predictions = {
        "train": load_prediction_records(
            args.train_predictions,
            [name for name in classes if name != args.background_class],
        ),
        "val": load_prediction_records(
