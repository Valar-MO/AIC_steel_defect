...
def main():
    ...
    balanced = HierarchicalDFINECriterion(
        matcher, {"loss_defectness": 1, "loss_class": 1}, ["hierarchical"],
        num_classes=9,
        objectness_class_weights=[1, 1, 1, 1, 1, 1, 0.75, 1, 1.25],
        class_priors=[0.20, 0.06, 0.06, 0.09, 0.10, 0.08, 0.30, 0.07, 0.04],
        class_logit_adjustment_tau=0.2,
    )
    assert balanced.objectness_class_weights[8] > balanced.objectness_class_weights[6]
    assert balanced.class_logit_adjustment_tau == 0.2
    assert torch.isclose(balanced.conditional_class_log_priors.exp().sum(), torch.tensor(1.0))
    ...
    if model.__class__.__name__ == "HierarchicalPEFTDFINE":
        report = model.peft_parameter_report()
        assert report["groups"]["adapter"] > 0
        assert report["trainable_fraction"] < 0.2
        assert not [name for name in report["trainable_names"]
