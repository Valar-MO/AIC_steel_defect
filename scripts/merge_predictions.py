        ...
        for source_name, records in sources:
            ...
            for pred in record.get("predictions", []):
                ...
                candidates[class_id].append({
                    ...
                    # Keep a stable pointer when raw predictions came from the
                    # ROI-refinement cache.  Ordinary prediction JSON has no such
                    # fields and remains byte-for-byte equivalent in behaviour.
                    "candidate_id": pred.get("candidate_id"),
                    "roi_cache_index": pred.get("roi_cache_index"),
                })
