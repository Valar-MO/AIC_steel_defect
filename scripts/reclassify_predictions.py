...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--score-fusion",
        choices=("soft", "multiply", "keep"),
        default="soft",
        help="soft = det_score * (0.5 + 0.5 * cls_prob).",
    )
    parser.add_argument(
        "--crop-expand",
        type=float,
...
def load_classifier(checkpoint: Path, dinov2_weights: Path, classes: list[str], device: torch.device):
    model = Dinov2CropClassifier(dinov2_weights, len(classes), dropout=0.0)
...
def main() -> None:
    ...
    unknown_targets = sorted(set(args.target_classes) - set(classes))
    if unknown_targets:
        raise ValueError(f"Unknown target classes: {unknown_targets}")
    unknown_frozen = sorted(set(args.frozen_source_classes) - set(classes))
    if unknown_frozen:
        raise ValueError(f"Unknown frozen source classes: {unknown_frozen}")
    unknown_blocked = sorted(set(args.blocked_target_classes) - set(classes))
    if unknown_blocked:
    ...
    model = load_classifier(args.classifier, args.dinov2_weights, classes, device)
    ...
    for item_index, probs in probs_by_index.items():
        ...
        original_prob = float(probs[original_id].item())
        ...
        new_score = fuse_score(old_score, final_prob, args.score_fusion)
