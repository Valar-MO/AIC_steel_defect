...
def map_tile_to_original(detection: dict[str, torch.Tensor], view_xyxy: torch.Tensor, local_size: int,
    ...
    ranking = detection["ranking_logits"].clone()
    margin = 2.0
    ...
    ranking[penalize] += float(torch.log(torch.tensor(internal_border_penalty)))
    return {"boxes": boxes, "labels": detection["labels"], "scores": detection["scores"], "ranking_logits": ranking}
...
def merge_tile_detections(tile_detections: list[dict[str, torch.Tensor]], soft_nms_iou: float = .80) -> dict[str, torch.Tensor]:
    ...
    if not nonempty:
        return {"boxes": exemplar["boxes"].new_zeros((0, 4)), "labels": exemplar["labels"].new_zeros((0,), dtype=torch.long),
                "scores": exemplar["scores"].new_zeros((0,)), "ranking_logits": exemplar["ranking_logits"].new_zeros((0,))}
